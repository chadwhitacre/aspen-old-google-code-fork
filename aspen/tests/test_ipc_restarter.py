...
def test_basic():
    ...
    assert_logs("done", force_unix_EOL=True)
...
def test_filesystem():
    ...
    def terminate_process(pid):
        """Terminate a process uncleanly.
        """
        import win32api
        handle = win32api.OpenProcess(1, 0, pid)
        win32api.TerminateProcess(handle, 0)

    def test(file_to_change, hack=False):
        mk( ('foo.py', '"bar"')
          , ('foo.conf', 'bar')
        ...
        # shut down both parent and child
        if not aspen.WINDOWS:
            ...
            if not hack:
                ...
                # deal with leftover log messages from last test run (i think?)
                # also, sometimes this just hangs :^(
                assert_logs( "restarting per should_restart"
                           , "child stopping"
                           , "parent started"
                           , "child started"
                           , "restarting per should_restart"
...
def _setup_signal_test():
    if aspen.WINDOWS:
        raise SkipTest # @@: how do we send a signal on Windows?
